...
import { Activity, Bell, Radio, User, Smartphone, Crown, HelpCircle } from 'lucide-react';
      ...
      <header className="sticky top-0 z-40 bg-[#0B0F19]/95 backdrop-blur border-b border-radar-border px-4 py-3 sm:px-6">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            {/* Guía de Dispositivos Button */}
            <button
              onClick={() => setIsGuideOpen(true)}
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-400 hover:text-cyan-300 border border-cyan-500/30 text-xs font-bold transition shadow-sm"
              title="Guía de Dispositivos y Sensores"
            >
              <HelpCircle className="w-4 h-4" />
              <span className="hidden sm:inline">Guía de Dispositivos</span>
            </button>
